...
<body>
  ...
  <input type='submit' value='Cari Data' class='btn btn-sm btn-primary'> <a href='index.php' class='btn btn-sm btn-success' > All Data</a>
   ...
   <form action="delete.php" method="POST" class="navbar-form navbar-left" role="search">
  <div class="form-group">
    <input type='text' placeholder='hapus Data Berdasarkan Kode dan Nama' name='kode' class='form-control'> <br/>
  </div>
  <input type='submit' value='hapus data' name="hapus" class='btn btn-sm btn-primary'>
    </form>
...
<br/>
  ...
  </div><div align="left" style="margin-left:12px;margin-top:12px"><a href="tambahpelanggan.php" class="table table-hover" >New Record</a></div>
  <div class="panel-body">
  
    <table class="table table-hover" >
      <th><td>ID PELANGGAN</a></td><td>NAMA</td><td>AlAMAT</td><td>EMAIL</td><td>NO.TELEPON</td><td>ACTION</td></th>

<?php
require_once('koneksi.php');
$query1="select * from tb_pelanggan ";


if(isset($_POST['qcari'])){
  $qcari=$_POST['qcari'];
  $query1="SELECT * FROM  tb_pelanggan
  where kd_plg like '%$qcari%'
  or nama like '%$qcari%'  ";
...
while($rows=mysqli_fetch_assoc($result)){
      ...
      <tr>
        <td><?php echo $no
        ?></td>
        <td><?php   echo $rows['kd_plg'];?></td>
        <td><?php   echo $rows['nama'];?></td>
        <td><?php   echo $rows['alamat'];?></td>
        <td><?php   echo $rows['email'];?></td>
        <td><?php   echo $rows['tlpn'];?></td>

        <td><a href="edit.php?kd_plg=<?php echo $rows['kd_plg']; ?>" class="btn btn-sm btn-success"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span> Edit Data</a></td>
